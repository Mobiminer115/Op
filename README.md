...
- `Khóa ngang game` cho phép game chạy landscape ngay cả khi khóa xoay hệ
  thống đang bật. iOS 16–18 dùng scene geometry request; iOS 12–15 dùng luồng
  xoay tương thích cũ. Thiết lập được lưu riêng trong từng app.
- Overlay không tự áp đặt portrait: mask orientation của menu luôn bám theo cửa
  sổ app chủ. App khai báo chỉ hỗ trợ landscape sẽ được giữ ngang tự động dù
  công tắc đang tắt.
- `Độ phân giải app` giảm backing resolution từ 100% xuống 10%, bước 5%,
...
- Mức 10–25% giảm rất mạnh số pixel và có thể làm chữ/hình ảnh trong game khó
  đọc; menu GameBoost vẫn giữ độ phân giải gốc để có thể đặt lại an toàn.
- Một app chỉ khai báo portrait có thể không bố trí giao diện đúng khi người dùng
  chủ động bật khóa ngang; chế độ này chủ yếu dành cho game có hỗ trợ landscape.
