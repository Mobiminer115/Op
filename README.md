...
- Liquid Glass tương thích iOS 12–18 bằng blur, gradient và viền màu. Đây là
  giao diện kính mô phỏng bằng API công khai, không phải framework giao diện của
  một phiên bản iOS tương lai.

## Thay đổi cần mở lại app

Game engine thường cache kích thước viewport và pipeline khi khởi động. Vì vậy
chuyển module, thay đổi downscale hoặc Super Resolution được lưu ngay nhưng chỉ
đổi framebuffer an toàn sau khi đóng hẳn rồi mở lại app. Menu hiển thị ký hiệu
`↻` khi đang chờ áp dụng. Filter sampler cũng chỉ tác động chắc chắn tới pipeline
được tạo sau khi module Enhance Graphic bật.
...
Workflow kiểm tra plist, source marker v2, kiến trúc dylib, framework liên kết và
